...
INSERT_FR_CFR_PARTS_SQL = """
INSERT INTO federal_register_cfr_parts (
    document_number,
    docket_id,
    cfr_title,
    cfr_part,
...
def build_cfr_rows(
    ...
    docket_ids = as_list_of_strings(doc.get("docket_ids"))
    cfr_refs = doc.get("cfr_references")
    if not docket_ids or not isinstance(cfr_refs, list) or not cfr_refs:
        return []

    valid_refs: list[tuple[int, str, str | None]] = []
    for ref in cfr_refs:
        ...
        if not is_numeric(title) or not is_numeric(part):
            continue
        citation_url = ref.get("citation_url")
        valid_refs.append((int(str(title)), str(part), citation_url))

    rows: list[tuple[Any, ...]] = []
    for docket_id in docket_ids:
        for title, part, citation_url in valid_refs:
            rows.append(
                (document_number, docket_id, title, part, citation_url)
            )
    return rows
